...
output = np.zeros((len(pfvalues), num_trials))
...
for i, pf in enumerate(pfvalues):
    for j in range(num_trials):
        res = siminf(A=A, pfreq=pf, 
            ...
            inference_method='SRC', ctime=ctime)
        output[i,j] = res['accuracy']
np.save('../data/extreme_events_ctime'+str(ctime)+'_T'+str(T)+'_idis'+str(int(idis))+'.npy', output)
